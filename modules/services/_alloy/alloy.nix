# Local vendored copy of the `alloy` service module proposed upstream in
# https://github.com/juspay/services-flake/pull/717.
# TODO: remove this file and consume the upstream `services.alloy` once
# https://github.com/juspay/services-flake/pull/717 is merged.
{
  pkgs,
  lib,
  name,
  ...
  config.outputs.settings.processes."${name}" = {
    ...
    readiness_probe = {
      http_get = {
        ...
        port = config.port or config.port;
        path = "/-/ready";
      };
