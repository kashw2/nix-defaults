...
}:
{
  options = {
    package = lib.mkOption {
      ...
      description = "The OneUptime probe package to run.";
    };

    port = lib.mkOption {
      type = lib.types.port;
      default = 3874;
      description = "Port the probe's HTTP server listens on.";
    };

    oneuptimeUrl = lib.mkOption {
      type = lib.types.str;
      default = "http://127.0.0.1:3002";
      description = "URL the probe reaches the OneUptime app on.";
    };
